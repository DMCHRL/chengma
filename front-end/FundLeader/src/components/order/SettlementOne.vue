    ...
    <div class="hui_content" id="hui-content">
			...
			<li class="item_box">
				...
				<div class="item_mid flex_bet" @click="toDetail(item)">
					...
					<div class="mid_box flex_one">
						<h4>{{item.mealList[0].name}}</h4>
						<p>{{item.mealList[0].include}}</p>
						<span>{{item.hotTag}}</span>
						<p>{{item.trainTime}}</p>
						<div class="clearfix">
              <span class="pull-right price">￥{{item.mealList[0].price}}</span>
            </div>
					</div>
				</div>
			</li>
      ...
      <!-- v-show="item.mealList[0].price > integral.balance" -->
		...
		<div class="sec_box sec_3">
			...
			<!-- <div v-show="item.mealList[0].price <= integral.balance" class="radio_item flex_bet" @click="isPointPay = !isPointPay">
				<span>积分支付（可用积分{{integral.balance}}）</span>
...
<script>
	...
	export default {
		...
		computed: {
          ...
          let total = this.item.mealList[0].price - this.voucher
          if (total < 0) {
          	return 0
          }else {
          	return total
					...
					return this.item.mealList[0].price
				}
			},
			payType () {
      ...
      voucher () {
        ...
        let sort = this.item.mealList[0].sort;//套餐类别
        let price = this.item.mealList[0].price;//商品价格
		...
		methods: {
          ...
          text: 'Loading'
				...
				if (_this.type == 'course') {
					url = '/api/hpp_train_apply/applyHppTrain';
				}else {
					url = '/api/hpp_exam_apply/applyHppExam';
				}
				
				
				_this.$post(url,datas).then((res) => {
			...
			toDetail (item) {
				...
				switch (_this.type){
					case 'course':
						_this.$router.push({
							path: '/tdetails?id'+item.id
						})
						break;
					case 'exam':
						_this.$router.push({
							path: '/examdetails?id'+item.id
						})
						break;
					default:
						break;
			...
			initPage() {
				...
				if (_this.type == 'exam') {
					//exam
					url = "/api/hpp_exam/findByCourseMealId/"
				}else {
					url = "/api/hpp_course/findByCourseMealId/"
				}
				
				_this.$fetch( url + _this.id).then((res) => {
