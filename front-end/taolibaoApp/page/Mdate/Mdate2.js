(function() {
	...
	Mdate.prototype = {
		...
		createDateUi: function() {
			var str = "" + '<section class="getDateBg"></section>' + '<section class="getDateBox" id="getDateBox">' + '<div class="choiceDateTitle">' + '<button id="dateCancel">取消</button>' + '<button id="dateSure" class="fr">確定</button>' + "</div>" + '<div class="dateContent">' + '<div class="checkeDate"></div>' + '<div style="width: 50%" id="yearwrapper">' + '<ul id="yearUl"></ul>' + "</div>" + '<div style="width: 40%;     left: 50%;" id="monthwrapper">' + '<ul id="monthUl"></ul>' + "</div>" + '<div style="display: none;" id="daywrapper">' + '<ul id="dayUl"></ul>' + "</div>" + "</div>" + "</section>";
			MdatePlugin.innerHTML = str
		},
		createDateYMD: function(type) {
			var that = this;
			var str = "<li>&nbsp;</li><li>全部</li>";
			var beginNum = null,
				endNum = null,
				unitName = "年",
				dataStyle = "data-year";
		...
		dateSure: function() {
			...
			sureBtn.addEventListener('click',function() {
				//点击确定
				sessionStorage.setItem('selectdate',JSON.stringify({'year':yearTag,'month':monthTag,'day':dayTag}));
				...
				//触发请求
				changeMonth(yearTag,that.dateForTen(monthTag));
