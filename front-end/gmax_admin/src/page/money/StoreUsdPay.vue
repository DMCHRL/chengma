...
<script>
	...
	import SavePayWay from "@/components/dialog/SavePayWay"
	...
	export default {
		...
		methods: {
			SaveSetting (val) {
				...
				_this.fetch("/api/receivables_en/changFlag/"+val,function (res) {
//					console.log(res)
...
//				console.log(item)
			...
			Delete (id) {
				...
				_this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
					...
					_this.fetch("/api/receivables_en/delete/"+id,function (res) {
						//console.log(res)
						if(res.statusCode == '0000') {
							_this.$message({
								type: 'success',
			...
			initPage () {
				...
				_this.fetch("/api/receivables_en/findAll",function (res) {
					// console.log(res)
					if (res.statusCode == "0000") {
