...
<script>
	import ReviewMessage from "@/components/dialog/ReviewMessage.vue"
	export default {
		data() {
			return {
				list: [], //任务列表
...
//				console.log(p)
			...
			Review(id) { //查看资料
				let _this = this;
				_this.fetch('/api//tlb-user/getTlbUser/' + id, function(res) {
					////console.log(res)
			...
			Delete(id, index) {
				...
				_this.$confirm('此操作将永久删除该条资料, 是否继续?', '提示', {
					...
					_this.fetch('/api//tlb-user/deleteTlbUser/' + id, function(res) {
//						console.log(res)
						if(res.statusCode == '0000') {
							_this.$message({
								type: 'success',
...
//				_this.loading = true;
				...
				_this.post('/api/tlb-user/pageList', datas, function(res) {
//					console.log(res)
