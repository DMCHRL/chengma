<template>
	<div class="content_box">
		...
		<div class="titling clearfix">
			<span>课程报名</span>
			<!--<button @click="saveShow = true">新增课程</button>-->
...
<script>
	...
	export default {
      ...
      formatMealInclude:function (val) {
		...
		methods: {
			outExe() {
              ...
              endTime: _this.filter.date[1],
					...
					_this.$postForExcel('/api/export/courseApply',datas,'交易面对面课程报名数据').then();
            ...
            endTime: _this.filter.date[1],
				...
				this.$post("/api/hpp_train_apply/pageList", datas).then((res) => {
					if (res.data) {
