	...
	<div class="content_box">
			...
			<div class="item_mess flex_row flex_align_center">
				...
				<div class="radio_groud flex_wrap">
					<template v-for="(item,index) in train.mealList">
						<span v-if="item.sort == 1" @click="change(index)" :class="[{'active':index == mealIndex},'radio_gui']">{{item.name}}</span>
					</template>
                    <div class="enter_line"></div>
                    <template v-for="(item,index) in train.mealList">
                    	<span v-if="item.sort == 2" @click="change(index)" :class="[{'active':index == mealIndex},'radio_gui']">{{item.name}}</span>
                    </template>
                    <div class="enter_line"></div>
                    <template v-for="(item,index) in train.mealList">
                    	<span v-if="item.sort == 3" @click="change(index)" :class="[{'active':index == mealIndex},'radio_gui']">{{item.name}}</span>
                    </template>
                    <div class="enter_line"></div>
                    <template v-for="(item,index) in train.mealList">
                    	<span v-if="item.sort > 3" @click="change(index)" :class="[{'active':index == mealIndex},'radio_gui']">{{item.name}}</span>
		...
		<div class="sec_box sec_2">
			...
			<div class="con_box">
				...
				<div class="flex_bet">
						<span class="name name_1">课程简介：</span>
...
<script>
	...
	export default {
		data() {
			return {
				headOption: {title: '课程详情',backText: '',showBack: true},
				train: {},
				mealIndex: 0,
				show12: false
			}
		...
		methods: {
			ToBuy () {
				this.$router.push({
					path: '/apply-train?id='+this.train.mealList[this.mealIndex].id
				})
			},
			change(i) {
				this.mealIndex = i;
