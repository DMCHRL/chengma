<template>
	<div class="content_box">
			...
			<div class="list_section">
				...
				<ul class="bodylist">
					<li class="clearfix" v-for = "item in list">
                        ...
                        <span class="pull-left">{{item.remarkNum}}</span>
                    ...
                    <li class="clearfix">
                    	<span class="pull-left">总计（当前页）</span>
                    	<span class="pull-left"></span>
                    	<span class="pull-left"></span>
                    	<span class="pull-left">{{totalAmount}}</span>
                    	<span class="pull-left"></span>
			...
			<!--审核弹窗-->
			<review-finance :dialogFormVisible="dialogFormVisible" :datas="dataReview" @close="dialogClose"></review-finance>
...
<script>
	import ReviewFinance from "@/components/dialog/ReviewFinance.vue"
	export default {
		data() {
			return {
				list: [],
		...
		methods: {
			...
			Review(id) { //查看资料
				let _this = this;
				_this.$until.superGet('/api/tlb-fund-apply/getTlbFundApply/' + id, function(res) {
			...
			initPage () {
				...
				_this.$until.superPost("/api/tlb-fund-apply/pageList",datas,function (res) {
                    // console.log(res)
					if (res.statusCode == "0000") {
