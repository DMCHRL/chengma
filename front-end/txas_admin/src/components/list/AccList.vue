<template>
	<div class="list_section">
      ...
      <span class="pull-left pointer" v-if="user.department == 'admin'" @click="Sort ('outTotal')">提现金额<i class="el-icon-d-caret"></i></span>
        ...
        <span class="pull-left" v-if="user.department == 'admin'">{{item.outTotal}}</span>
...
//				console.log(id)
				...
				_this.$confirm('此操作将账户密码重置为默认, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					_this.$until.superGet("/api/tlb-account/resetPassword/" + id, function(res) {
						//console.log(res)
