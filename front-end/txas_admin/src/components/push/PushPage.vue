		...
		methods: {
			Delete(id) {
				...
				_this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
					...
					_this.$until.superGet("/api/notice/delete/" + id, function(res) {
						//console.log(res)
			...
			sendMessage (id) {
				...
				_this.$until.superPost("/api/notice/sendNotice",datas,function (res) {
					// console.log(res)
					if (res.statusCode == "0000") {
						_this.$message({
							message: '发送成功',
			...
			initPage () {
				...
				_this.$until.superPost("/api/notice/pageList",datas,function (res) {
					// console.log(res)
					if (res.statusCode == "0000") {
