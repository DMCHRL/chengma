...
//			},
			renderItem(params, api) {
				...
				var pointLeftTop = coords[key] || (coords[key] = api.coord([+(latExtent[0] + lngIndex * cellSizeCoord[0]).toFixed(6), +(lngExtent[0] + latIndex * cellSizeCoord[1]).toFixed(6)]));
				var pointRightBottom = coords[key] || (coords[key] = api.coord([+(latExtent[0] + (lngIndex + 1) * cellSizeCoord[0]).toFixed(6), +(lngExtent[0] + (latIndex + 1) * cellSizeCoord[1]).toFixed(6)]));
...
<style scoped>
	#map {
		background-image: url(http://tlb.txasfx.com/crm/img/bg_111.png);
