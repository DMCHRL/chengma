...
public class TlbAccountServiceImpl implements TlbAccountService {
    ...
    @Autowired
    private TlbUserService tlbUserService;

    @Autowired
    private TlbUserMapper tlbUserMapper;
    ...
    @Override
    public HashMap<String, Object> createTlbAccount(TlbAccountDTO tlbAccountDTO) {
        HashMap<String, Object> retMap = new HashMap<>();
        TlbAccountDTO tlbAccountDTO1 = save(tlbAccountDTO);
        if(mt4Service.createMT4Acount(tlbAccountDTO1,tlbUserMapper.toEntity(tlbUserService.findOneByUserId(userService.getUserWithAuthorities().getId())))!=0){
            retMap.put("statusCode", ResponseResult.FAIL_CODE);
            retMap.put("msg", "创建失败");
            return retMap;
        }
        retMap.put("statusCode", ResponseResult.SUCCESS_CODE);
        retMap.put("data",tlbAccountDTO1);
        return retMap;
    }
    ...
    @Override
    public int saveReal(TlbAccountDTO tlbAccountDTO, TlbUser tlbUser) {
        return  mt4Service.createMT4Acount(tlbAccountDTO,tlbUser);
    }
