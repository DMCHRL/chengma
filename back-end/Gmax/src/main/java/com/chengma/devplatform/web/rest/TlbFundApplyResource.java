...
public class TlbFundApplyResource {
    ...
    @PostMapping("/tlb-fund-apply/checkRecharge")
    @Timed
    public ResponseEntity<ResponseResult> checkRecharge(@RequestBody HashMap<String, Object> params) {
        log.debug("REST request to get a page of TlbFundApply");
        HashMap<String, Object> checkMap = tlbFundApplyService.checkRecharge(params);
        ResponseResult json = new ResponseResult();
        if(!ResponseResult.SUCCESS_CODE.equals(checkMap.get("statusCode"))){
            json.setMsgCode(checkMap.get("msg").toString());
            return new ResponseEntity<>(json, null, HttpStatus.OK);
    ...
    public ResponseEntity<ResponseResult> pass(@PathVariable String id) {
        ...
        ResponseResult json = new ResponseResult();
        HashMap<String, Object> checkMap = tlbFundApplyService.checkPassOrReject();
        if(!ResponseResult.SUCCESS_CODE.equals(checkMap.get("statusCode"))){
            json.setMsgCode(checkMap.get("msg").toString());
            return new ResponseEntity<>(json, null, HttpStatus.OK);
        }
