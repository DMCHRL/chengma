...
public class UserService {
    ...
    public ResponseResult save(UserDTO userDTO) {
        ...
        if (null == login || "".equals(login)) {
            ...
            response.setMsgCode(DevplatformConstants.USER_INVALID_MOBILE);
            return response;
      ...
      /*  } else if (null == expireDate) {
       ...
       /* } else if (null == passwordExpireDate) {
        ...
        User uMobile = findUserByMobile(userDTO.getMobile());
    ...
    public HashMap<String, Object> findAll(HashMap<String, Object> params) {
        ...
        StringBuilder column = new StringBuilder(" select u.id, u.login, u.c_mobile, u.c_del_flag, u.c_password_remark, u.c_parent_id, u.c_eara,\n" +
                " u.department, u.email, u.activated, u.image_url, u.first_name, u.created_date, u.lang_key, u.activation_key, \n" +
                " u.c_status,u.reset_key, u.reset_date, u.last_modified_by, u.last_modified_date, u.c_char_no, u.c_sex \n");
        column.append(" ,case u.department when '"+ EnumRole.ADMIN.value() + "' then '" + EnumRoleName.admin.value() + "'");
        column.append("   when '"+ EnumRole.ACCOUNT.value() + "' then '" + EnumRoleName.account.value() + "'");
        column.append("   when '"+ EnumRole.COMPANY.value() + "' then '" + EnumRoleName.company.value() + "'");
        column.append("   when '"+ EnumRole.PROXY.value() + "' then '" + EnumRoleName.proxy.value() + "'");
        column.append("   when '"+ EnumRole.SERVICE.value() + "' then '" + EnumRoleName.service.value() + "'");
        column.append("   else '"+ EnumRoleName.user.value() + "' end last_name ");
        column.append(" ,case u.activated when '1' then '" + Constants.STATUS_ENABLE + "' else '" + Constants.STATUS_DISABLE + "' end activated_show ");
        column.append(" ,case when u.d_expire_date < " + dbService.sysdate() + " or u.d_password_expire_date < " + dbService.sysdate() + " then '1' else '0' end is_expire ");
        column.append(", ifnull(ua.account_count, 0) account_count");
        column.append(", ifnull(ua.parent_name, '') parent_name");
        column.append(", ifnull(up.proxy_count, 0) proxy_count");

        StringBuilder cond = new StringBuilder(" from jhi_user u ");
        cond.append(" left join  (" + accountTableCountStr() + ") ua on u.id = ua.top_id\n");
        cond.append(" left join  (" + proxyTableCount() + ") up on u.id = up.user_id\n");

        cond.append(" where 1 = 1 and u.c_del_flag = '1'");
        ...
        if (StringUtils.isNotBlank(mobile)) {
            cond.append(" and u.c_mobile like '%" + mobile + "%'");
        }
        if (StringUtils.isNotBlank(email)) {
            cond.append(" and u.email like '%" + email + "%'");
        }
        if (StringUtils.isNotBlank(activatedStr)) {
            cond.append(" and u.activated = '" + activatedStr + "'");
        }
        if (StringUtils.isNotBlank(department)) {
            cond.append(" and u.department = '" + department + "'");
        }
        if (StringUtils.isNotBlank(isProxy) && "Y".equals(isProxy)) {
            cond.append(" and u.department in ('" + EnumRole.PROXY + "', '" + EnumRole.COMPANY +"')");
        }

        String orderBy="";
        if (StringUtils.isNotBlank(orderByColumn)) {
            String columnName="";
            if(StringUtil.lineToHump("account_count").equals(orderByColumn)){
                columnName="account_count";
            }else if(StringUtil.lineToHump("parent_name").equals(orderByColumn)){
                columnName="parent_name";
            }else if(StringUtil.lineToHump("proxy_count").equals(orderByColumn)){
                columnName="proxy_count";
            }else{
                columnName="u."+ ReflectUtils.getColumnName(User.class,orderByColumn);
            }
            orderBy = SqlUtil.orderBySql(columnName,sort);
        }

        Page<UserDTO> page = pageCommon.execPage(column.toString(), cond.toString(),orderBy, page_number, page_size, UserDTO.class);
    ...
    public List<TlbAccountDTO> loadMyAccount(){
        ...
        StringBuilder stringBuilder = new StringBuilder("select a.* from t_tlb_account a  where a.c_user_id = '"+userId+"'");
