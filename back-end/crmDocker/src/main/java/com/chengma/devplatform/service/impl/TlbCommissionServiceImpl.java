...
public class TlbCommissionServiceImpl implements TlbCommissionService {
    ...
    public TlbCommissionDTO updateCommissionBalance(String account, double amount) {
        ...
        newBanlance = new BigDecimal(newBanlance).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
    	if(newBanlance<0){
    		return null;
    	}else{
	...
	@Override
	public void updateTotal(String accountId, double lots) {
		...
		if(tlbAccountDTO!=null&&tlbAccountDTO.getUserId()!=null){
            ...
            if(EnumRole.COMPANY.value().equals(userDTO.getDepartment())){
                ...
                tlbCommissiondto.setTotal(tlbCommissiondto.getTotal() + lots * tlbCommissiondto.getLotAmount());
                tlbCommissiondto.setBalance(tlbCommissiondto.getBalance()+ lots * tlbCommissiondto.getLotAmount());
            ...
            //代理自己客户
                ...
                //计算公司代理佣金
                calcCompanyProxy(pTlbCommissiondto, tlbCommissiondto, lots);
            //普通客户
                ...
                if( parentUserDTO != null ) {
                    ...
                    if (EnumRole.PROXY.value().equals(parentUserDTO.getDepartment())) {
                        ...
                        calcCompanyProxy(companyCommissiondto, proxyCommissiondto, lots);
                    //挂在公司下面
    ...
    public void updateCommission(){
        ...
        for(TlbTradeDTO tDTO:tibTradeListDTO){
            updateTotal(tDTO.getAccount(),tDTO.getLots());
            tDTO.setCalcSum("Y");
            tlbTradeService.save(tDTO);
        }
    }

    private void calcCompanyProxy(TlbCommissionDTO companyCommissionDTO, TlbCommissionDTO proxyCommissionDTO, double lots){
        if(proxyCommissionDTO != null) {
            double proxyAmount = lots * proxyCommissionDTO.getLotAmount();
            double companyAmount =  lots * (proxyCommissionDTO.getLotAmount() - proxyCommissionDTO.getLotAmount());
            //更新代理佣金
            proxyCommissionDTO.setTotal(proxyCommissionDTO.getTotal() + proxyAmount);
            proxyCommissionDTO.setBalance(proxyCommissionDTO.getBalance()+proxyAmount);
            tlbCommissionRepository.save(tlbCommissionMapper.toEntity(proxyCommissionDTO));
            //更新公司佣金
            companyCommissionDTO.setTotal(companyCommissionDTO.getTotal() + companyAmount);
            companyCommissionDTO.setBalance(companyCommissionDTO.getBalance() + companyAmount);
