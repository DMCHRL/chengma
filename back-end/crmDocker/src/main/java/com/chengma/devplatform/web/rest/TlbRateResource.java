...
/**
 * REST controller for managing TlbRate.
 */
...
public class TlbRateResource {
    ...
    public ResponseEntity<ResponseResult> pageList(@ApiParam Pageable pageable) {
        log.debug("REST request to get a page of TlbRates");
    ...
    @Timed
    public ResponseEntity<ResponseResult> createTlbRate(@RequestBody TlbRateDTO tlbRateDTO) throws URISyntaxException {
        log.debug("REST request to save TlbRate : {}", tlbRateDTO);
    ...
    public ResponseEntity<ResponseResult> updateSysUser(@RequestBody TlbRateDTO tlbRateDTO) throws URISyntaxException {
        ...
        if (tlbRateDTO.getId() == null) {
            return createTlbRate(tlbRateDTO);
        }
    ...
    @Timed
    public ResponseEntity<ResponseResult> getTlbRate(@PathVariable String id) {
        log.debug("REST request to get TlbRate : {}", id);
        ResponseResult json = new ResponseResult();
        try {
    ...
    public ResponseEntity<ResponseResult> deleteSysUser(@PathVariable String id) {
        log.debug("REST request to delete TlbRate : {}", id);
    ...
    @GetMapping("/tlb-rate/realTimeRate")
    @Timed
    public ResponseEntity<ResponseResult> getTlbRate() {

        //XXPayUtil.call4Post(EnvUtil.getProperty("rate.url"));
        ResponseResult json = new ResponseResult();
        json.setData(XXPayUtil.call4Post(EnvUtil.getProperty("rate.url")));
        /*try {
            TlbRateDTO sysComponentDTO = tlbRateService.findOne(id);
            json.setData(sysComponentDTO);
            json.setStatusCode(ResponseResult.SUCCESS_CODE);
