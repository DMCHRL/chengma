...
public class TlbHistoryKServiceImpl implements TlbHistoryKService {
    ...
    public Page<TlbHistoryKDTO> pageList(HashMap<String, Object> params) {
        ...
        if (StringUtils.isNotBlank(bTime)) {
            cond.append(" and TO_DAYS(d_time) >= TO_DAYS('"+bTime+"') ");
        }
        if (StringUtils.isNotBlank(eTime)) {
            cond.append(" and TO_DAYS(d_time) <= TO_DAYS('"+eTime+"') ");
        }
     ...
     /*  String orderBy="";
        if (StringUtils.isNotBlank(orderByColumn)) {
            String columnName= ReflectUtils.getColumnName(TlbHistoryK.class,orderByColumn);
