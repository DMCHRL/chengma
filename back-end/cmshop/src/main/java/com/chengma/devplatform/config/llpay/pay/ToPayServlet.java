package com.chengma.devplatform.config.llpay.pay;
...
public class ToPayServlet extends HttpServlet {
	...
	private void plainPay(HttpServletRequest req, OrderInfo order) throws Exception
    {
		...
		paymentInfo.setId_type("0");   //可选
		paymentInfo.setId_no(req.getParameter("id_no")); //可选
    ...
    private OrderInfo createOrder(HttpServletRequest req)
    {
        ...
        orderInfo.setNo_order(LLPayUtil.getCurrentDateTimeStr());  //商户订单号。
        orderInfo.setDt_order(LLPayUtil.getCurrentDateTimeStr());  //商户订单时间。
        orderInfo.setMoney_order(req.getParameter("money_order")); //交易金额。
        orderInfo.setName_goods(req.getParameter("name_goods"));  //商户商品名称。 可选
        orderInfo.setInfo_order("用户购买" + req.getParameter("name_goods")); //订单扩展字段。 可选
        return orderInfo;
    }
