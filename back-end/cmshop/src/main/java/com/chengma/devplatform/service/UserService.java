...
public class UserService {
    ...
    public ResponseResult save(UserDTO userDTO) {
        ...
        if(userRepository.findOneByLogin(userDTO.getLogin()) != null){
            response.setMsgCode("用户名已存在");
            response.setStatusCode(ResponseResult.FAIL_CODE);
            return response;
        }
        if(userDTO.getPasswordRemark().length() < 6){
            response.setMsgCode("密码长度不能少于6");
            response.setStatusCode(ResponseResult.FAIL_CODE);
            return response;
        }

        User user = userMapper.userDTOToUser(userDTO);
        user.setPassword(passwordEncoder.encode(userDTO.getPasswordRemark()));
        user.setPasswordRemark(userDTO.getPasswordRemark());
        user.setActivated(true);
        user.setDelFlag("0");
        user.setDepartment(EnumRole.USER.value());
        user.setLastName(EnumRoleName.user.value());
        userRepository.save(user);
