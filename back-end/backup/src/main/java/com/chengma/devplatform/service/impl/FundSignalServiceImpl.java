...
public class FundSignalServiceImpl implements FundSignalService {
    ...
    public Page<FundSignalDTO> pageList(HashMap<String, Object> params) {
        ...
        StringBuilder cond = new StringBuilder(" from t_fund_signal ");
    ...
    public List<FindListFundSignalDTO> findList(HashMap<String, Object> params) {
        ...
        StringBuffer sb = new StringBuffer("select s.c_id,s.c_name,s.i_year_profit1 as year_profit1 ,s.i_year_profit2 as year_profit2 ,i_risk risk,DATEDIFF( s.d_end_at,s.d_operation_at) as cycle_time,DATEDIFF( s.d_operation_at,NOW()) as remaining_time, (select ROUND(ifnull(sum(r.i_money),0)*100/s.i_target_fund,4) from t_fund_record r where r.c_fund_signal_id = s.c_id) as progress ");
