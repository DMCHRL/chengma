...
*/
     ...
     *
     * @param userId 参数：{userId:"",activated:""}
     * @return statusCode:成功0000，失败0100。
     */
    @GetMapping("/user/loadInfo/{userId}")
    @Timed
    public ResponseEntity<ResponseResult> loadInfo(@PathVariable String userId) {
        log.debug("REST request to createPowerUser {}",userId);
        MobileUserDTO mobileUserDTO = userService.loadInfo(userId);
        ResponseResult json = new ResponseResult();
