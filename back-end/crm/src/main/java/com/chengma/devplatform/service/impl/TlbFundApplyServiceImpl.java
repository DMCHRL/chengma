...
public class TlbFundApplyServiceImpl implements TlbFundApplyService {
    ...
    public Page<TlbFundApplyDTO> pageList(HashMap<String, Object> params) {
        ...
        //非admin或非service 只能看到自己的记录
        if(!(user.getDepartment().equals(EnumRole.ADMIN.value()) || user.getDepartment().equals(EnumRole.SERVICE.value()))){
            cond = new StringBuilder("  FROM\n" +
                    "\tt_tlb_fund_apply t\n" +
                    "JOIN t_tlb_account c ON t.c_account = c.c_account\n" +
                    "JOIN jhi_user b ON c.c_user_id = b.id\n" +
                    "LEFT JOIN jhi_user b2 ON b2.id = t.c_approved_id\n" +
                    "WHERE\n" +
                    "\t1 = 1 ");
        }else{
            cond = new StringBuilder("  from t_tlb_fund_apply t join jhi_user b on t.c_user_id = b.id left join jhi_user b2 on b2.id = t.c_approved_id where 1 = 1 ");
        }
        ...
        if(user.getDepartment() != null){
            //非admin或非service 只能看到自己的记录
            if(!(user.getDepartment().equals(EnumRole.ADMIN.value()) || user.getDepartment().equals(EnumRole.SERVICE.value()))){
                cond.append("and b.id = '"+user.getId()+"' ");
            }
        }else{
            return null;
        }
    ...
    public TlbFundApplyDTO recharge(HashMap<String, Object> params) {
        ...
        String bankNum = params.get("bankNum") == null ? "" : params.get("bankNum").toString();
        String remarkNum = params.get("remarkNum") == null ? "" : params.get("remarkNum").toString();
        ...
        tlbFundApplyDTO.setStatus(DevplatformConstants.FUND_APPLY_STATUS_APPLYING);
        tlbFundApplyDTO.setUserId(tlbAccountDTO.getUserId());
        tlbFundApplyDTO.setBankNum(bankNum);
        tlbFundApplyDTO.setRemarkNum(remarkNum);
    ...
    @Override
    public HashMap<String, Object> checkRecharge(HashMap<String, Object> params) {
        HashMap<String, Object>  retMap = new HashMap<>();
        if(null == params.get("usd") ||  Double.valueOf(params.get("usd").toString())>7000){
            retMap.put("statusCode", ResponseResult.FAIL_CODE);
            retMap.put("msg", "最大單筆入金為$7000");
            return retMap;
        }
    ...
    public TlbFundApplyDTO reject(String id) {
        ...
        if(DevplatformConstants.FUND_APPLY_INNER.equals(tlbFundApplyDTO.getFundType())) {
            mailService.sendRejectFundInnerMail(userService.getUserWithAuthorities(tlbAccountDTO.getUserId()), tlbAccountDTO);
        }else if(DevplatformConstants.FUND_APPLY_OUT.equals(tlbFundApplyDTO.getFundType())) {
            mailService.sendRejectFundOutMail(userService.getUserWithAuthorities(tlbAccountDTO.getUserId()), tlbAccountDTO);
        }/*else if(DevplatformConstants.FUND_APPLY_IN.equals(tlbFundApplyDTO.getFundType())) {
            mailService.sendRejectFundInMail(userService.getUserWithAuthorities(tlbAccountDTO.getUserId()), tlbAccountDTO);
        }*/
    ...
    @Override
    public HashMap<String, Object> checkPassOrReject() {
