...
public class TreeCreator {
	...
	public static <T extends TreeNode> List<T> constructTree(List<T> list, Long root) {
		return constructTree(list, root, null);
	}
	
	public static <T extends TreeNode> List<T> constructTree(List<T> list, Long root, String attributes) {
		...
		for(T tn : list) {
			if(root == null) {
				...
				if(tn.getParentId().longValue() == root.longValue()) {
					tree.add(tn);
				}
			}
		}
