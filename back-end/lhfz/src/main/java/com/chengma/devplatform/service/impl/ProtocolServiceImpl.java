...
public class ProtocolServiceImpl implements ProtocolService {
    ...
    public HashMap<String, Object> checkSave(ProtocolDTO protocolDTO) {
        ...
        if(DevplatformConstants.ABOUT_US.equals(protocolDTO.getType())){
            if(StringUtils.isBlank(protocolDTO.getQq())){
                retMap.put("statusCode", ResponseResult.FAIL_CODE);
                retMap.put("msg", "请输入客服QQ");
                return retMap;
            }
            if(StringUtils.isBlank(protocolDTO.getPhone())){
                retMap.put("statusCode", ResponseResult.FAIL_CODE);
                retMap.put("msg", "请输入客服电话");
                return retMap;
            }
        }
