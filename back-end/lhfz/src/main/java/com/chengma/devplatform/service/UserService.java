...
public class UserService {
    ...
    public ResponseResult save(UserDTO userDTO) {
        ...
        try {
          ...
          /*  while(mobileUserService.findByRecommendation(recommendationNo) != null && this.findByCharNo(recommendationNo) != null){
    ...
    public HashMap<String, Object> findAll(HashMap<String, Object> params) {
       ...
       /* String orderByColumn = formParams.get("orderByColumn") == null ? "" : formParams.get("orderByColumn").toString();
        ...
        StringBuilder column = new StringBuilder(" select u.id,u.login,case when u.department='partner'  or u.department='originator' or u.department='manager' then m.c_name else u.first_name end as first_name,u.department, u.last_name");
        column.append(",m.c_id_positive,m.c_id_negative,ifnull(u.email,m.c_email) as email,m.c_password,m.d_create_at,m.d_update_at,m.c_status,m.c_head_img, m.c_mobile");

        StringBuilder cond = new StringBuilder(" from jhi_user u LEFT JOIN t_lhfz_mobile_user m on u.id = m.c_user_id where u.c_del_flag");
      ...
      /*  String orderBy="";
        ...
        Page<UserListDTO> page = pageCommon.execPage(column.toString(), cond.toString(), page_number, page_size, UserListDTO.class);
...
/*    public HashMap<String, Object> findAllAccount(HashMap<String, Object> params) {
    ...
    public HashMap<String,Object> createPowerUser(MobileUserDTO mobileUserDTO){
        HashMap<String,Object> result = new HashMap<>();

        if(StringUtils.isBlank(mobileUserDTO.getMobile())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请输入手机号码");
            return  result;
        }

        if(StringUtils.isBlank(mobileUserDTO.getEmail())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请输入邮箱");
            return  result;
        }

        if(StringUtils.isBlank(mobileUserDTO.getIdPositive())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请上传身份证正面");
            return  result;
        }

        if(StringUtils.isBlank(mobileUserDTO.getIdNegative())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请上传身份证背面");
            return  result;
        }

        if(StringUtils.isBlank(mobileUserDTO.getDepartment())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请选择角色");
            return  result;
        }

        if(StringUtils.isBlank(mobileUserDTO.getName())){
            result.put("statusCode", ResponseResult.FAIL_CODE);
            result.put("msg", "请输入昵称");
            return  result;
        }

        if(!(DevplatformConstants.USER_ROLE_MANAGER.equals(mobileUserDTO.getDepartment()) || DevplatformConstants.USER_ROLE_ORIGINATOR.equals(mobileUserDTO.getDepartment()))){
            mobileUserDTO.setDepartment(DevplatformConstants.USER_ROLE_PARTNER);
        }
        User dbUser=new User();
        dbUser.setLogin(mobileUserDTO.getMobile());
        dbUser.setMobile(mobileUserDTO.getMobile());
        dbUser.setPassword(DevplatformConstants.USER_DEFAULT_PASSWORD);
        dbUser.setDepartment(mobileUserDTO.getDepartment());
        ...
        //初始化基本信息
        mobileUserDTO.setStatus(DevplatformConstants.MOBILE_USER_AUTHORIZED);
        mobileUserDTO.setUserId(dbUser.getId());
        mobileUserService.initMobileUser(mobileUserDTO);
    ...
    加载用户信息
     */
    public MobileUserDTO loadInfo(){
        String sql =  "select u.c_id, u.c_head_img, u.c_name, u.c_mobile, u.c_id_positive, u.c_id_negative, u.c_email, CASE WHEN c_password IS NULL THEN 0 ELSE 1 END AS password, u.d_create_at, u.d_update_at, u.c_status, u.c_cid, u.c_user_id, u.i_level,i.department,a.i_balance,a.i_total,a.i_used,a.i_net_profit,a.i_fund_profit from t_lhfz_mobile_user u,t_asset a,jhi_user i where  u.c_user_id=a.c_user_id and i.id =u.c_user_id and u.c_user_id='"+userService.getUserWithAuthorities().getId()+"'";
        return baseDao.findListBySql(sql,MobileUserDTO.class).get(0);
    }
   ...
   加载投资人信息
    */
    public MobileUserDTO loadPowerInfo(HashMap<String,Object> params){
        ...
        String sqlDepartment = "c_originator_id";
       /* if(DevplatformConstants.USER_ROLE_ORIGINATOR.equals(department)){
            sqlDepartment = ""+sqlDepartment+"";
        }*/
        if(DevplatformConstants.USER_ROLE_MANAGER.equals(department)){
            sqlDepartment = "c_manager_id";
        }
        String sql =  "SELECT\n" +
                "\tu.*, i.department,\n" +
                "\tIFNULL(a.avg_year_rate, 0) AS avg_year_rate,\n" +
                "\tIFNULL(a.total_profit, 0) AS total_profit,\n" +
                "\tIFNULL(a.success_rate, 0) AS success_rate,\n" +
                "\tIFNULL(a.finish, 0) AS finish,\n" +
                "\tIFNULL(a.profit_num, 0) AS profit_num,\n" +
                "\tIFNULL(a.total_pre_fund, 0) AS total_pre_fund,\n" +
                "\tIFNULL(a.avg_pre_fund, 0) AS avg_pre_fund\n" +
                "FROM\n" +
                "\tjhi_user i,\n" +
                "\tt_lhfz_mobile_user u\n" +
                "LEFT JOIN (\n" +
                "\tSELECT\n" +
                "\t\td."+sqlDepartment+" AS "+sqlDepartment+",\n" +
                "\t\tsum(\n" +
                "\t\t\tRound(\n" +
                "\t\t\t\t(d.i_profit - d.i_pre_fund) * 100 / d.i_pre_fund,\n" +
                "\t\t\t\t4\n" +
                "\t\t\t)\n" +
                "\t\t) / COUNT(*) AS avg_year_rate,\n" +
                "\t\tsum(d.i_profit - d.i_pre_fund) AS total_profit,\n" +
                "\t\tcount(*) * 100 / (\n" +
                "\t\t\tSELECT\n" +
                "\t\t\t\tcount(*)\n" +
                "\t\t\tFROM\n" +
                "\t\t\t\tt_fund_signal s\n" +
                "\t\t\tWHERE\n" +
                "\t\t\t\ts."+sqlDepartment+" = d."+sqlDepartment+"\n" +
                "\t\t) AS success_rate,\n" +
                "\t\tcount(*) AS finish,\n" +
                "\t\tsum(\n" +
                "\t\t\tCASE\n" +
                "\t\t\tWHEN (d.i_profit - d.i_pre_fund > 0) THEN\n" +
                "\t\t\t\t1\n" +
                "\t\t\tELSE\n" +
                "\t\t\t\t0\n" +
                "\t\t\tEND\n" +
                "\t\t) profit_num,\n" +
                "\t\tsum(d.i_pre_fund) total_pre_fund,\n" +
                "\t\t(\n" +
                "\t\t\tifnull(sum(d.i_pre_fund), 0) / count(*)\n" +
                "\t\t) AS avg_pre_fund\n" +
                "\tFROM\n" +
                "\t\tt_fund_signal_data d\n" +
                "\tWHERE\n" +
                "\t\td.c_status = 'FINISH'\n" +
                "\tGROUP BY\n" +
                "\t\td."+sqlDepartment+"\n" +
                ") a ON a."+sqlDepartment+" = u.c_user_id\n" +
                "WHERE\n" +
                "\ti.id = u.c_user_id\n" +
                "AND u.c_user_id = '"+departmentId+"'";
    ...
    树状图数据
     */
    public UserChartDTO showUserChart(String userId){
        StringBuffer sb = new StringBuffer("select a.current_fund , b.originator_fund ,c.manager_fund ");
        sb.append(" from ( select IFNULL(sum(r.i_money),0) as current_fund from t_fund_record r join t_fund_signal s on s.c_id=r.c_fund_signal_id and s.c_status !='FINISH'  where r.c_user_id='"+userId+"') a"); //当前入伙金额
        sb.append(", (select IFNULL(sum(r.i_money),0) as originator_fund from t_fund_record r join t_fund_signal s on s.c_id=r.c_fund_signal_id and s.c_status !='FINISH'  where s.c_originator_id='"+userId+"')b");                                  //发起金额
        sb.append(", (select IFNULL(sum(r.i_money),0)  as manager_fund from t_fund_record r join t_fund_signal s on s.c_id=r.c_fund_signal_id and s.c_status !='FINISH'  where s.c_manager_id='"+userId+"') c");   //资金管理
        return baseDao.findListBySql(sb.toString(),UserChartDTO.class).get(0);
    }



    public List<UserComboListDTO> findUserComboList(HashMap<String, Object> params){
        StringBuilder column = new StringBuilder(" select u.id,u.first_name,u.department");
        column.append(" from jhi_user u ");
        return baseDao.findListBySql(column.toString(),UserComboListDTO.class);
    }
