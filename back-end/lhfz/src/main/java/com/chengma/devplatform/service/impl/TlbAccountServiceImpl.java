...
public class TlbAccountServiceImpl implements TlbAccountService {
    ...
    public Page<TlbAccountDTO> pageList(HashMap<String, Object> params) {
       ...
       /* String account = formParams.get("account") == null ? "" : formParams.get("account").toString();
        ...
        StringBuilder column = new StringBuilder(" select a.* ,u.first_name");
    ...
    public HashMap<String, Object> checkCreateTlbAccountDTO(TlbAccountDTO tlbAccountDTO) {
        ...
        if(StringUtils.isBlank(tlbAccountDTO.getAccountName())){
            retMap.put("statusCode",ResponseResult.FAIL_CODE);
            retMap.put("msg", "请输入账号名称");
            return  retMap;
        }

        if(StringUtils.isBlank(tlbAccountDTO.getMt4Password())){
            retMap.put("statusCode",ResponseResult.FAIL_CODE);
            retMap.put("msg", "请输入交易密码");
            return  retMap;
        }

        if(StringUtils.isBlank(tlbAccountDTO.getUserId()) || null == userService.getUserWithAuthorities(tlbAccountDTO.getUserId())){
            retMap.put("statusCode",ResponseResult.FAIL_CODE);
            retMap.put("msg", "请选择归属人");
            return  retMap;
        }
    ...
    @Override
    public HashMap<String, Object> editPassword(HashMap<String, Object> params) {
        HashMap<String, Object> result = new HashMap<>();
        String account = params.get("account") == null? "" :  (String)params.get("account");
        String oldPassword = params.get("oldPassword") == null? "" :  (String)params.get("oldPassword");
        String newPassword = params.get("newPassword") == null? "" :  (String)params.get("newPassword");

        TlbAccount tlbAccount = tlbAccountRepository.findOneByAccountEqualsAndMt4PasswordEquals(account,oldPassword);
        if(null == tlbAccount){
            result.put("statusCode",ResponseResult.FAIL_CODE);
            result.put("msg", "账号或密码错误");
            return  result;
        }
        tlbAccount.setMt4Password(newPassword);
        tlbAccountRepository.save(tlbAccount);
        result.put("statusCode",ResponseResult.SUCCESS_CODE);
        return result;
    }

    @Override
    public List<AccountComboListDTO> accountComboList(String account) {
        StringBuilder column = new StringBuilder(" select c_account,c_account_name");
        column.append(" from t_tlb_account  where 1=1 ");
        if(StringUtils.isNotBlank(account)){
            column.append(" and c_account like '%"+account+"%'");
        }
        return baseDao.findListBySql(column.toString(),AccountComboListDTO.class);
    }
}
