...
public class TlbAccountResource {
     ...
     * @return
     */
    @PostMapping("/tlb_account/editPassword")
    @Timed
    public ResponseEntity<ResponseResult> editPassword(@RequestBody HashMap<String,Object> params){
        ResponseResult json = new ResponseResult();
        log.debug("REST request to editPassword of a TlbAccount : {}", params);
        HashMap<String, Object> checkMap = tlbAccountService.editPassword(params);
        if(!ResponseResult.SUCCESS_CODE.equals(checkMap.get("statusCode"))){
            json.setMsgCode(checkMap.get("msg").toString());
            return new ResponseEntity<>(json, null, HttpStatus.OK);
        }
     ...
     * @return statusCode:成功0000，失败0100。
     */
    @GetMapping("/tlb_account/accountComboList/{account}")
    @Timed
    public ResponseEntity<ResponseResult> accountComboList(@PathVariable(required = false) String account) {
        log.debug("REST request to accountComoboList  TlbAccount ");
        List<AccountComboListDTO> list = tlbAccountService.accountComboList(account);
        ResponseResult json = new ResponseResult();
        HashMap<String,Object> result = new HashMap<>();
        result.put("list",list);
        json.setData(result);
