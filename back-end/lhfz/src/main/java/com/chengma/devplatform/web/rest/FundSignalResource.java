...
public class FundSignalResource {
     ...
     * @return
     */
    @GetMapping("/fund_signal/preFundJoin/{id}")
    @Timed
    public ResponseEntity<ResponseResult> preFundJoin(@PathVariable String id){
        ResponseResult json = new ResponseResult();
        log.debug("REST request to preFundJoin of FundSignal : {}", id);
        PreFundJoinDTO preFundJoinDTO = fundSignalService.preFundJoin(id);
        json.setStatusCode(ResponseResult.SUCCESS_CODE);
        json.setData(preFundJoinDTO);
        return new ResponseEntity<>(json, null, HttpStatus.OK);
    }
